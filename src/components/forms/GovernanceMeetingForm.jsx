// src/components/forms/GovernanceMeetingForm.jsx
...
import { 
    INITIAL_GOVERNANCE_MEETING_STATE,
    GOVERNANCE_MEETING_TYPE_OPTIONS
} from '../../utils/constants.js'; 
import { getDbPaths } from '../../services/firebase.js'; 
import { useTranslation } from '../../context/TranslationContext.jsx';

const GovernanceMeetingForm = ({ userId, db, mode = 'add', initialData = null, onClose, role }) => {
    const { t } = useTranslation();
    const isAdmin = role === 'admin';
    
    const [formData, setFormData] = useState(initialData || INITIAL_GOVERNANCE_MEETING_STATE);
    ...
    const formTitle = mode === 'edit' 
        ? t('governance.meeting.form.edit_title')
        : t('governance.meeting.form.add_title');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.error(`Error ${mode} governance meeting: `, error);
        ...
        <div className="rounded-2xl border border-sky-700/50 bg-black/40 shadow-2xl backdrop-blur-lg overflow-hidden max-w-4xl mx-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <InputField 
                        label={t('governance.meeting.col.name')} 
                        name="name" 
                    ...
                    />
                    <SelectField 
                        label={t('governance.meeting.col.type')} 
                        name="type" 
                        options={GOVERNANCE_MEETING_TYPE_OPTIONS.map(opt => ({ value: opt, label: t(`governance.meeting.type.${opt.toLowerCase()}`) }))} 
                        value={formData.type} 
                        onChange={handleChange} 
                        disabled={!isAdmin}
                    />
                ...
                <InputField 
                    label={t('governance.meeting.col.date')} 
                    name="date" 
                    type="date"
...
export default GovernanceMeetingForm;