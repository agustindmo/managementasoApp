...
import { INITIAL_PROJECT_STATE } from '../../utils/constants.js'; 
import { getDbPaths } from '../../services/firebase.js'; 
import { useTranslation } from '../../context/TranslationContext.jsx';

const FinanceProjectForm = ({ userId, db, mode = 'add', initialData = null, onClose, role }) => { 
    ...
    const formTitle = mode === 'edit' ? t('finance.projects.form_edit') : t('finance.projects.form_add');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.error(`Error ${mode} project: `, error);
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-xl overflow-hidden max-w-4xl mx-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <InputField label={t('finance.col.sponsor')} name="sponsor" value={String(formData.sponsor ?? '')} onChange={handleChange} disabled={!isAdmin} />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <InputField label={t('finance.col.amount')} name="amount" type="number" value={String(formData.amount ?? '')} onChange={handleChange} required={true} disabled={!isAdmin} />
                </div>
                <InputField label={t('finance.col.description')} name="description" value={String(formData.description ?? '')} onChange={handleChange} required={false} rows={2} disabled={!isAdmin} />
                {isAdmin && (
