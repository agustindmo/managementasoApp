// src/components/tables/FinanceServiceTable.jsx

import React, { useState, useEffect, useMemo } from 'react';
import { ref, onValue, remove } from 'firebase/database';
import { Loader2, Briefcase, ArrowUp, ArrowDown, Download, PlusCircle, Edit, Trash2 } from 'lucide-react';
import { getDbPaths } from '../../services/firebase.js';
import CardTitle from '../ui/CardTitle.jsx';
import { 
    ALL_FILTER_OPTION,
    SERVICE_TABLE_COLUMNS,
    SERVICE_COLUMN_OPTIONS_MAP
...
// --- Fila de la Tabla ---
const ServiceTableRow = ({ item, onEdit, onDelete, t }) => {
    return (
        <tr className="hover:bg-sky-900/60 transition-colors">
            <td className="px-6 py-3 text-sm font-medium text-white truncate max-w-[200px]" title={item.name}>{item.name}</td>
...
const FinanceServiceTable = ({ db, onOpenForm }) => {
    const { t } = useTranslation(); 
    
    const dbPathKey = 'financeServices';
    const filterOptionsMap = SERVICE_COLUMN_OPTIONS_MAP;
    const titleKey = 'finance.tab.services';
    const addKey = 'finance.services.form_add';
    ...
    useEffect(() => {
        ...
        const unsubscribe = onValue(dataRef, (snapshot) => {
            ...
            } catch (e) { console.error("Error processing Service Log snapshot:", e); }
            finally { setIsLoading(false); }
        }, (error) => { console.error("Service Log Subscription Error:", error); setIsLoading(false); });
    ...
    const { filteredAndSortedItems, totalSum } = useMemo(() => {
        let finalData = dataItems.filter(item => {
            for (const column of SERVICE_TABLE_COLUMNS) {
    ...
    const handleDelete = async (id) => {
        if (db && window.confirm(t('policy.confirm_delete'))) { 
            ...
            } catch (e) { console.error("Error deleting service document:", e); }
        }
    };
    ...
    const handleDownloadXLSX = () => {
        ...
        const columns = SERVICE_TABLE_COLUMNS;
                ...
                <table className="min-w-full divide-y divide-sky-800/50">
                    ...
                    <tbody className="bg-sky-950/50 divide-y divide-sky-800/50">
                        {filteredAndSortedItems.length > 0 ? (
                            ...
                            <tr><td colSpan={SERVICE_TABLE_COLUMNS.length} className="px-6 py-4 text-center text-gray-500">{t('press_log.no_records')}</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
...
export default FinanceServiceTable;