...
import { Loader2, Gift, ArrowUp, ArrowDown, Download, PlusCircle, Edit, Trash2 } from 'lucide-react';
...
const TableHeaderWithControls = ({ column, currentSort, onSortChange, onFilterChange, filterOptions, currentFilters, t }) => {
    ...
    let options = [];
    if (column.filterable && column.optionsKey) options = filterOptions[column.optionsKey] || [];
...
const DonationTableRow = ({ item, onEdit, onDelete, t, isAdmin }) => (
    <tr className="hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0">
        <td className="px-6 py-3 text-sm font-medium text-slate-900">{item.donor}</td>
        ...
        <td className="px-6 py-3 text-sm text-slate-600 truncate max-w-[200px]" title={item.description}>{item.description}</td>
...
const FinanceDonationTable = ({ db, onOpenForm, role }) => { 
    ...
    useEffect(() => {
        ...
        const unsubscribe = onValue(dataRef, (snapshot) => {
            ...
            catch (e) { console.error("Error processing Donation snapshot:", e); } 
            finally { setIsLoading(false); }
        }, (error) => { console.error("Donation Subscription Error:", error); setIsLoading(false); });
        return () => unsubscribe();
    }, [db]);
    ...
    const filteredAndSortedItems = useMemo(() => {
        let finalData = dataItems.filter(item => {
            for (const column of DONATION_TABLE_COLUMNS) {
                ...
                if (column.optionsKey && itemValue !== filterValue) return false;
                else if (!String(itemValue).toLowerCase().includes(String(filterValue).toLowerCase())) return false;
            }
            return true;
        });
        if (sort.key) {
    ...
    const handleDelete = async (id) => {
        if (db && isAdmin && window.confirm(t('policy.confirm_delete'))) { 
            try { await remove(ref(db, `${getDbPaths().financeDonations}/${id}`)); } 
    ...
    if (isLoading) return <div className="flex justify-center items-center h-48"><Loader2 className="w-8 h-8 text-blue-500 animate-spin" /><p className="ml-3 text-slate-500">{t('finance.donations.loading')}</p></div>;
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            <div className="flex items-center justify-between p-4 bg-white border-b border-slate-200">
                <div className="flex items-center space-x-3">
                    ...
                    <h2 className="text-sm font-semibold text-slate-700 uppercase tracking-wider">{`${t('finance.tab.donations')} (${filteredAndSortedItems.length})`}</h2>
                </div>
                <div className="flex space-x-2">
                    <button onClick={handleDownloadXLSX} className="flex items-center space-x-2 bg-white text-slate-700 border border-slate-300 px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-slate-50 transition shadow-sm"><Download className="w-4 h-4" /><span>{t('policy.download_xlsx')}</span></button>
                    {isAdmin && <button onClick={() => onOpenForm(null)} className="flex items-center space-x-2 bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-blue-700 transition shadow-md"><PlusCircle className="w-4 h-4" /><span>{t('finance.donations.add')}</span></button>}
                </div>
            </div>
                    ...
                    <thead className="bg-slate-50">
                        <tr>
                            {DONATION_TABLE_COLUMNS.map(column => (
                                <TableHeaderWithControls key={column.key} column={column} currentSort={sort} onSortChange={handleSortChange} onFilterChange={handleFilterChange} filterOptions={DONATION_COLUMN_OPTIONS_MAP} currentFilters={filters} t={t} />
                            ))}
                        </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-slate-200">
                        {filteredAndSortedItems.length > 0 ? filteredAndSortedItems.map(item => <DonationTableRow key={item.id} item={item} onEdit={() => onOpenForm(item)} onDelete={() => handleDelete(item.id)} t={t} isAdmin={isAdmin} />) : <tr><td colSpan={DONATION_TABLE_COLUMNS.length} className="px-6 py-4 text-center text-slate-500">{t('press_log.no_records')}</td></tr>}
                    </tbody>
                </table>
            </div>
        </div>
