// src/components/tables/FinanceProviderTable.jsx

import React, { useState, useEffect, useMemo } from 'react';
import { ref, onValue, remove } from 'firebase/database';
import { Loader2, Truck, ArrowUp, ArrowDown, Download, PlusCircle, Edit, Trash2, Link } from 'lucide-react';
import { getDbPaths } from '../../services/firebase.js';
import CardTitle from '../ui/CardTitle.jsx';
import { 
    ALL_FILTER_OPTION,
    PROVIDER_TABLE_COLUMNS,
    PROVIDER_COLUMN_OPTIONS_MAP
...
// --- Fila de la Tabla ---
const ProviderTableRow = ({ item, onEdit, onDelete, t }) => {
    const dates = item.isContinued ? `${item.startDate} - ${t('finance.donations.col.isContinued')}` : `${item.startDate} / ${item.endDate || 'N/A'}`;
    ...
    return (
        <tr className="hover:bg-sky-900/60 transition-colors">
            ...
            <td className="px-6 py-3 text-sm text-gray-400">{item.type}</td>
            <td className="px-6 py-3 text-sm text-gray-400">{item.productService}</td>
            <td className="px-6 py-3 text-sm text-gray-400 truncate max-w-[200px]" title={item.serviceProvided}>{item.serviceProvided}</td>
            <td className="px-6 py-3 text-sm text-green-400 font-semibold">{formatCurrency(item.amount)}</td>
            <td className="px-6 py-3 text-sm text-gray-400 whitespace-nowrap" title={dates}>{dates}</td>
...
const FinanceProviderTable = ({ db, onOpenForm }) => {
    const { t } = useTranslation(); 
    
    const dbPathKey = 'financeProviders';
    const filterOptionsMap = PROVIDER_COLUMN_OPTIONS_MAP;
    const titleKey = 'finance.providers.title';
    const addKey = 'finance.providers.form_add';
    ...
    useEffect(() => {
        ...
        const unsubscribe = onValue(dataRef, (snapshot) => {
            ...
            } catch (e) { console.error("Error processing Provider Log snapshot:", e); }
            finally { setIsLoading(false); }
        }, (error) => { console.error("Provider Log Subscription Error:", error); setIsLoading(false); });
            ...
            for (const column of PROVIDER_TABLE_COLUMNS) {
                ...
                if (key === 'isContinued') {
                    itemValue = itemValue ? 'Yes' : 'No';
                     ...
                     itemValue = item.isContinued ? `${item.startDate} continued` : `${item.startDate} ${item.endDate || ''}`;
                }
    ...
    const handleDelete = async (id) => {
        if (db && window.confirm(t('policy.confirm_delete'))) { 
            ...
            } catch (e) { console.error("Error deleting provider document:", e); }
        }
    };
    ...
    const handleDownloadXLSX = () => {
        ...
        const columns = PROVIDER_TABLE_COLUMNS;
            ...
            columns.filter(col => col.key !== 'actions').forEach(col => {
                ...
                switch(col.key) {
                    case 'amount':
                        ...
                        break;
                    case 'startDate':
                        value = item.isContinued ? `${item.startDate} - ${t('finance.donations.col.isContinued')}` : `${item.startDate} / ${item.endDate || 'N/A'}`;
                        break;
                    case 'isContinued':
                        value = item.isContinued ? t('profile.yes') : t('profile.no');
                        break;
                    case 'link':
                        value = item.link ? 'View Link' : 'N/A';
                        break;
                    default:
                        value = item[col.key] || '';
                }
                ...
                <table className="min-w-full divide-y divide-sky-800/50">
                    ...
                    <tbody className="bg-sky-950/50 divide-y divide-sky-800/50">
                        {filteredAndSortedItems.length > 0 ? (
                            ...
                            <tr><td colSpan={PROVIDER_TABLE_COLUMNS.length} className="px-6 py-4 text-center text-gray-500">{t('press_log.no_records')}</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
